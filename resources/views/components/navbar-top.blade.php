<div x-data="{open_side_menu:false, show_explore:false}" class="fixed z-40 h-20 ">
    <div class="relative">
        ...
        <div id="navbar" class="w-screen h-20 px-8 navbar navbar-transparent">
            ...
            {{-- button side menu --}}
            <button @click="open_side_menu = ! open_side_menu"
                class="w-12 h-12 p-2 text-white bg-white bg-opacity-25 focus:outline-none lg:hidden">
        ...
        <div class="absolute inset-0 z-50 w-screen h-screen overflow-hidden" x-cloak x-show="open_side_menu">
            <div class="relative h-full w-ful">
                ...
                {{-- dark mask --}}
                <div class="absolute inset-0 h-full bg-black bg-opacity-75" x-show="open_side_menu"
                    x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0"
                    x-transition:enter-end="opacity-90" x-transition:leave="ease-in duration-300"
                ...
                <div class="absolute top-0 left-0 w-4/5 h-full max-w-xs p-6 bg-white"
                    @click.away="open_side_menu = false" x-show="open_side_menu"
                    x-transition:enter="transition ease-out duration-300"
                    x-transition:enter-start="transform -translate-x-full"
                    x-transition:enter-end="transform translate-x-0" x-transition:leave="ease-in duration-300"
