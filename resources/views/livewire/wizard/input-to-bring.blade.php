<div class="pb-8">
    ...
    <h1 class="font-extrabold text-3xl mb-2">Que deberán traer tus invitados</h1>
    ...
    <div class="mt-8">
        ...
        <div class="relative mt-2 h-14 w-full">
            <x-ui.input wire:model="item" class="h-full" id="item" name="item" placeholder="Agrega los items que deben traer" />
            <button wire:click="addItem"
                class="block absolute inset-y-0 right-4 p-1 m-auto h-10 w-10 rounded-full outline-none hover:shadow-w1 transition ease-in-out text-white bg-success" title="Agregar">
                <x-icon-add-outline />
            </button>
        </div>
    ...
    <h2 class="mt-8 font-semibold text-xl border-b pb-2">Lista de Requerimientos</h2>
    <div class="pl-2">

        @if (count($toBring)>=1)
        <ol class="list-decimal list-inside text-sm mt-1">
            @foreach ($toBring as $item)
            <li class="flex py-2 pl-2 mb-4 border-b ">
                ...
                <div class="w-1/5 flex">
                    <button class="ml-auto mb-auto mr-2 text-gray-500 hover:text-secondary"
                        wire:click="deleteItem({{ $loop->index }})">Eliminar</button>
                </div>
            </li>
            @endforeach
        </ol>
